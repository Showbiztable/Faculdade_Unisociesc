...
void gerar_carros(void *arg)//tempo de execução = 7500 ns
{
	RTIME inicio, fim;
	...
	while(1){ 	
		...
		if(tempo_atual_do_sistema - tempo_anterior_thread_2 > 1e8){
			...
			for(i = 0; i < LIMITE_DA_FILA; i++){
				...
				//printf(" Fila %d = %d ",i,filas[i].num_carros);
...
void fluxo(void *arg)//tempo varia entre 9000 da primeira vez e 600 na segunda
{
	...
	while(1){
		rt_sem_p(&sem_fluxo,TM_NONBLOCK);

		inicio = rt_timer_read();	
		int tempo_atual_do_sistema = rt_timer_read();

		if(tempo_atual_do_sistema - tempo_anterior_thread_4 > 5e8){
...
{
	rt_mutex_create(&mutex_filas,"Mutex_Filas");
	...
	rt_sem_create(&sem_fluxo, "Semaforo", 1, S_PRIO);
