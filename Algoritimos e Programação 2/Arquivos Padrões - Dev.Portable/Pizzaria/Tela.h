...
public:
	...
	char perguntar(string pergunta, int linha)
	{
		char resposta;
		this->centralizar(pergunta, linha);
		resposta = getche ();
		resposta = toupper(resposta); // Cai na prova; string não combina com toupper
		return resposta;
	}	
	...
	string mostrarMenuBarras(int colini, int linini, vector<string> opcoes)
	{
		...
		colfim = colini+opcoes[0].size()+1; // Primeira opção do seu vetor e seu tamanho
